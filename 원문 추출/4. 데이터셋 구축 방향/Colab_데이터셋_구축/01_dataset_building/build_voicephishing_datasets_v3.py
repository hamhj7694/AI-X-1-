...
CLAIM_CUE_PATTERN = re.compile(
    r"(?:저|저희|제가|나는|여기는|본인은).{0,20}(?:소속|담당|직원|수사관|검사|경찰관|형사|팀장|과장).{0,12}(?:입니다|이라고\s*합니다|라고\s*합니다|이고요|인데요)"
    r"|(?:은행|검찰청|지검|경찰청|경찰서|금융감독원|금감원|카드사|캐피탈|저축은행).{0,15}(?:소속|담당|직원|수사관|검사|경찰관|형사|팀장|과장).{0,12}(?:입니다|이고요|인데요)"
    r"|(?:은행|검찰청|지검|경찰청|경찰서|금융감독원|금감원|카드사|캐피탈|저축은행)입니다",
    re.IGNORECASE,
)
...
AMOUNT_OFFER_PATTERN = re.compile(
    r"(?:드리겠습니다|드릴게요|지급해드리|지급해\s*드리|입금해드리|"
    r"받으실\s*수|받게\s*되|환급받|지원금|대출해드리|승인해드리|한도를\s*드리)",
    re.IGNORECASE,
)
AMOUNT_REQUEST_DIRECTION_PATTERN = re.compile(
    r"(?:보내세요|보내\s*주|이체하세요|이체\s*해|입금하세요|입금\s*해|"
    r"납부하세요|납부\s*해|준비하세요|준비\s*해|인출하세요|출금하세요|"
    r"전달하세요|전달\s*해|안전계좌|보증금|수수료|공증료|발급비|인지대)",
    re.IGNORECASE,
)
AMOUNT_PURPOSE_RULES = [
    ("REFUND", re.compile(r"환급|환불|지원금|보상금|돌려")),
    ("FEE", re.compile(r"수수료|공증료|발급비|인지대|처리비|비용")),
    ("TAX", re.compile(r"세금|국세|지방세|납부")),
    ("SECURITY_DEPOSIT", re.compile(r"보증금|보증료|예치금|안전계좌")),
    ("LOAN_PRINCIPAL", re.compile(r"대출|대출\s*원금|한도|승인금액|대출승인")),
    ("ACCOUNT_BALANCE", re.compile(r"잔액|잔고|통장에|계좌에")),
]
...
def inventory_fingerprint(paths: list[Path], root: Path) -> str:
    """대용량 원본을 다시 읽지 않고 파일명·크기·수정시각으로 실행 입력을 식별한다."""
    records = []
    for path in sorted(paths):
        ...
        records.append(f"{relative_path}|{stat.st_size}|{stat.st_mtime_ns}")
...
def classify_amount_direction(text: str, role: str, amount_status: str) -> str:
    """금액을 주겠다는 제안과 돈을 요구하는 표현을 분리하는 SILVER 규칙."""
    text = normalize_text(text)
    if role == "VICTIM" and amount_status == "CLAIMED_COMPLETED":
        return "VICTIM_CLAIMED_SENT"
    if role == "VICTIM" and amount_status == "AGREED":
        return "VICTIM_AGREED_TO_PAY"
    if role == "OFFENDER" and AMOUNT_OFFER_PATTERN.search(text):
        return "OFFERED_TO_VICTIM"
    if role == "OFFENDER" and (
        amount_status == "REQUESTED"
        or AMOUNT_REQUEST_DIRECTION_PATTERN.search(text)
    ):
        return "REQUESTED_FROM_VICTIM"
    return "NO_DIRECTION"


def classify_amount_purpose(text: str) -> str:
    """금액이 어떤 명목으로 언급되었는지 분류하는 SILVER 규칙."""
    text = normalize_text(text)
    for purpose, pattern in AMOUNT_PURPOSE_RULES:
        if pattern.search(text):
            return purpose
    return "UNKNOWN"


def amount_direction_confidence(direction: str, amount_status: str) -> float:
    if direction in {
        "OFFERED_TO_VICTIM", "REQUESTED_FROM_VICTIM",
        "VICTIM_AGREED_TO_PAY", "VICTIM_CLAIMED_SENT",
    }:
        return 0.80 if amount_status in {"REQUESTED", "AGREED", "CLAIMED_COMPLETED"} else 0.65
    return 0.45
...
def extract_amount_events(case_turns: list[dict]) -> list[dict]:
    ...
    for turn in case_turns:
        ...
        for match_index, match in enumerate(matches, start=1):
            ...
            amount_direction = classify_amount_direction(
                text, turn["auto_role"], amount_status
            )
            amount_purpose = classify_amount_purpose(text)
            row = {
                ...
                "amount_purpose": amount_purpose,
                "amount_direction_evidence": text,
                "amount_direction_confidence": amount_direction_confidence(
                    amount_direction, amount_status
                ),
        ...
        if not matches and turn["auto_role"] == "VICTIM" and recent_request:
            ...
            if 0 < turn_gap <= 2 and (is_completed or is_agreed):
                ...
                amount_direction = (
                    "VICTIM_CLAIMED_SENT" if is_completed else "VICTIM_AGREED_TO_PAY"
                )
                rows.append({
                    ...
                    "amount_status": amount_status,
                    "amount_direction": amount_direction,
                    "amount_purpose": classify_amount_purpose(
                        recent_request["evidence_text"]
                    ),
                    "amount_direction_evidence": text,
                    "amount_direction_confidence": 0.70,
    ...
    columns = [
        ...
        "amount_status", "amount_direction", "amount_purpose",
        "amount_direction_evidence", "amount_direction_confidence",
        "related_action_type", "evidence_text", "inferred_from_context",
...
def aggregate_impersonations(hits: list[dict]) -> pd.DataFrame:
    columns = [
        ...
        "evidence_text", "institution_role", "case_candidate_score", "candidate_rank",
        "primary_confidence_tier", "case_review_required",
        "extraction_method", "extraction_confidence", "extraction_version", "label_status",
    ]
    if not hits:
    ...
    for key, group_hits in grouped.items():
        ...
        rows.append({
            ...
            "institution_role": "CANDIDATE",
            "case_candidate_score": 0.0,
            "candidate_rank": 0,
            "primary_confidence_tier": "CANDIDATE",
    ...
    # 발화에서 후보를 찾되, 최종 주요 사칭은 사건 안의 모든 후보를 함께 비교한다.
    # 명시적 자기소개가 없거나 후보 점수가 비슷하면 하나를 강제로 선택하지 않는다.
    missing_tokens = {"", "NOT_MENTIONED", "UNKNOWN", "NONE", "NAN"}
    for _, group in result.groupby("case_id"):
        group_index = group.index
        explicit_claim = (
            group["identity_claim_type"].eq("CALLER_SELF_CLAIM")
            & group["evidence_role"].eq("OFFENDER")
        )
        offender_mention = group["evidence_role"].eq("OFFENDER")
        has_role_title = ~group["claimed_role_title_normalized"].fillna("").astype(str).str.upper().isin(missing_tokens)
        has_org_name = ~group["claimed_org_name_normalized"].fillna("").astype(str).str.upper().isin(missing_tokens)
        repeated = np.log1p(pd.to_numeric(group["mention_count"], errors="coerce").fillna(0)).clip(upper=2.0)

        score = (
            explicit_claim.astype(float) * 4.0
            + offender_mention.astype(float) * 2.0
            + has_role_title.astype(float) * 2.0
            + has_org_name.astype(float) * 1.0
            + repeated
            - (~group["identity_claim_type"].eq("CALLER_SELF_CLAIM")).astype(float) * 1.5
        )
        result.loc[group_index, "case_candidate_score"] = score
        result.loc[group_index, "institution_role"] = np.select(
            [explicit_claim, offender_mention],
            ["EXPLICIT_SELF_CLAIM", "OFFENDER_MENTION"],
            default="OTHER_SPEAKER_MENTION",
        )

        ranked = group.assign(_score=score).sort_values(
            ["_score", "mention_count", "first_mention_turn"],
            ascending=[False, False, True],
        )
        for rank, index in enumerate(ranked.index, start=1):
            result.loc[index, "candidate_rank"] = rank

        high = ranked[
            explicit_claim.reindex(ranked.index).fillna(False)
            & ranked["_score"].ge(6.0)
        ]
        medium = ranked[
            offender_mention.reindex(ranked.index).fillna(False)
            & (
                has_role_title.reindex(ranked.index).fillna(False)
                | pd.to_numeric(ranked["mention_count"], errors="coerce").fillna(0).ge(2)
            )
            & ranked["_score"].ge(3.5)
        ]

        pool = high if not high.empty else medium
        tier = "HIGH" if not high.empty else ("MEDIUM" if not medium.empty else "UNKNOWN")
        selected_index = None
        margin = np.inf
        if not pool.empty:
            selected_index = pool.index[0]
            if len(pool) > 1:
                margin = float(pool.iloc[0]["_score"] - pool.iloc[1]["_score"])
            # 두 후보의 점수가 거의 같으면 사건 전체에서도 주 사칭을 확정하기 어렵다.
            if margin < 0.75:
                selected_index = None
                tier = "UNKNOWN"

        review_required = tier != "HIGH" or group["impersonation_subtype"].nunique() > 1
        result.loc[group_index, "case_review_required"] = bool(review_required)
        if selected_index is not None:
            result.loc[selected_index, "is_primary_impersonation"] = True
            result.loc[selected_index, "primary_confidence_tier"] = tier
        else:
            result.loc[group_index, "primary_confidence_tier"] = "UNKNOWN"
    return result
...
def enrich_cases(cases_df, impersonations_df, actions_df, strategies_df):
    ...
    if not impersonations_df.empty:
        ...
        primary = primary[[
            ...
            "primary_confidence_tier", "case_candidate_score", "evidence_text",
        ]].rename(columns={
            ...
            "primary_confidence_tier": "primary_impersonation_confidence_tier",
            "case_candidate_score": "primary_impersonation_score",
            "evidence_text": "primary_impersonation_evidence",
        })
        ...
        summary_rows = []
        for case_id, group in impersonations_df.groupby("case_id"):
            ordered = group.sort_values(["first_mention_turn", "candidate_rank"])
            all_subtypes = list(dict.fromkeys(ordered["impersonation_subtype"].dropna().astype(str)))
            explicit_subtypes = list(dict.fromkeys(
                ordered.loc[ordered["institution_role"].eq("EXPLICIT_SELF_CLAIM"), "impersonation_subtype"]
                .dropna().astype(str)
            ))
            selected = ordered[ordered["is_primary_impersonation"]]
            selected_subtype = selected.iloc[0]["impersonation_subtype"] if not selected.empty else "UNKNOWN"
            secondary = [value for value in all_subtypes if value != selected_subtype]
            transition = []
            for value in ordered["impersonation_subtype"].dropna().astype(str):
                if not transition or transition[-1] != value:
                    transition.append(value)
            summary_rows.append({
                "case_id": case_id,
                "access_impersonation_subtype": all_subtypes[0] if all_subtypes else "NOT_MENTIONED",
                "secondary_impersonation_subtypes": " | ".join(secondary) if secondary else "NOT_MENTIONED",
                "mentioned_impersonation_subtypes": " | ".join(all_subtypes) if all_subtypes else "NOT_MENTIONED",
                "explicit_claim_subtypes": " | ".join(explicit_subtypes) if explicit_subtypes else "NOT_MENTIONED",
                "impersonation_transition_sequence": " -> ".join(transition) if transition else "NOT_MENTIONED",
                "impersonation_review_required": bool(ordered["case_review_required"].fillna(True).any()),
                "impersonation_label_source": "RULE_CASE_LEVEL_V3",
            })
        case_summary = pd.DataFrame(summary_rows)
        result = (
            result.merge(primary, on="case_id", how="left")
            .merge(counts, on="case_id", how="left")
            .merge(case_summary, on="case_id", how="left")
        )

        # 후보는 있지만 확정 기준을 통과한 주요 사칭이 없는 사건은 UNKNOWN으로 보존한다.
        candidate_case = result["case_id"].isin(set(impersonations_df["case_id"]))
        for column in [
            "primary_impersonation_group", "primary_impersonation_subtype",
            "primary_claimed_org_name", "primary_claimed_role_title",
            "primary_impersonation_confidence_tier", "primary_impersonation_evidence",
        ]:
            result.loc[candidate_case & result[column].isna(), column] = "UNKNOWN"
        result.loc[candidate_case & result["primary_impersonation_score"].isna(), "primary_impersonation_score"] = 0.0
    ...
    defaults = {
        ...
        "primary_impersonation_confidence_tier": "NOT_MENTIONED",
        "primary_impersonation_score": 0.0,
        "primary_impersonation_evidence": "NOT_MENTIONED",
        "access_impersonation_subtype": "NOT_MENTIONED",
        "secondary_impersonation_subtypes": "NOT_MENTIONED",
        "mentioned_impersonation_subtypes": "NOT_MENTIONED",
        "explicit_claim_subtypes": "NOT_MENTIONED",
        "impersonation_transition_sequence": "NOT_MENTIONED",
        "impersonation_review_required": False,
        "impersonation_label_source": "RULE_CASE_LEVEL_V3",
...
def build_fraud_detection_ml(cases_df: pd.DataFrame, normal_df: pd.DataFrame) -> pd.DataFrame:
    ...
    normal_rows = pd.DataFrame({
        ...
        "quality_flag": "USABLE",
        "original_split": selected_normal["dataset_split"],
        "label_source": "AIHUB_OFFICIAL_SPLIT",
    })
...
def build_segment_detection_ml(cases_df, utterances_df, normal_df, fraud_detection_df):
    ...
    for conversation_id in selected_normal_ids:
        ...
        rows.append({
            ...
            "fraud_label": "LEGITIMATE_FINANCIAL_CALL",
            "original_split": item.get("dataset_split", "UNKNOWN"),
            "label_source": "AIHUB_OFFICIAL_SPLIT",
            "sample_scope": "FULL",
            "window_start_turn": 1,
            "window_end_turn": len(turns),
...
def build_korean_case_summary(files_df, cases_df, impersonations_df, actions_df, amounts_df, dashboard_df=None):
    ...
    korean_result = result.reindex(columns=korean_columns).rename(columns=rename)

    # 사람이 확인하는 대시보드 표에는 영문 파생변수도 모두 한글 컬럼으로 붙인다.
    # 영문 표는 ML·코딩용으로 그대로 유지하고, 이 표만 보고서·대시보드용으로 사용한다.
    if dashboard_df is not None and not dashboard_df.empty:
        dashboard_rename = {
            "file_id": "파일ID", "case_id": "사건ID", "source_category": "원본분류코드",
            "source_file": "원본파일", "case_start_sec": "사건시작_초", "case_end_sec": "사건종료_초",
            "duration_sec": "통화시간_초", "turn_count": "전체발화수", "speaker_count": "화자수",
            "offender_turn_count": "범인추정발화수", "victim_turn_count": "피해자추정발화수",
            "unknown_turn_count": "역할미분류발화수", "unknown_role_ratio": "역할미분류비율",
            "short_turn_count": "짧은발화수", "short_turn_ratio": "짧은발화비율",
            "raw_full_text": "전체대화", "normalized_full_text": "전체대화_정규화",
            "raw_offender_text": "범인추정대화", "normalized_offender_text": "범인추정대화_정규화",
            "raw_victim_text": "피해자추정대화", "normalized_victim_text": "피해자추정대화_정규화",
            "asr_avg_logprob": "평균전사로그확률", "needs_review": "검수필요여부",
            "quality_flag": "품질상태", "label_status": "역할라벨상태",
            "supervised_target": "지도학습대상여부", "primary_impersonation_group": "주요사칭대분류코드",
            "primary_impersonation_subtype": "주요사칭세부유형코드", "primary_claimed_org_name": "주요사칭기관명",
            "primary_claimed_role_title": "주요사칭직책코드", "impersonation_count": "사칭표현수",
            "primary_impersonation_confidence_tier": "주요사칭신뢰등급",
            "primary_impersonation_score": "주요사칭자동점수",
            "primary_impersonation_evidence": "주요사칭근거문장",
            "access_impersonation_subtype": "접근사칭세부유형코드",
            "secondary_impersonation_subtypes": "보조사칭세부유형목록",
            "mentioned_impersonation_subtypes": "전체언급사칭세부유형목록",
            "explicit_claim_subtypes": "명시적자기사칭유형목록",
            "impersonation_transition_sequence": "사칭전환순서",
            "impersonation_review_required": "사칭검수필요여부",
            "impersonation_label_source": "사칭라벨출처",
            "impersonation_group_count": "사칭대분류종류수", "primary_requested_action": "주요요구행동코드",
            "requested_action_count": "요구행동표현수", "risky_action_diversity": "위험행동종류수",
            "first_risky_action_turn": "최초위험행동발화순번", "first_risky_action_sec": "최초위험행동시점_초",
            "mentioned_amount_max_krw": "최대언급금액_원", "risky_action_count": "위험행동수",
            "authority_trust_count": "권위신뢰_횟수", "behavior_control_count": "행동통제_횟수",
            "benefit_promise_count": "이익제안_횟수", "fear_threat_count": "공포위협_횟수",
            "information_extraction_count": "정보수집_횟수", "isolation_secrecy_count": "고립비밀유지_횟수",
            "legitimacy_building_count": "정당성형성_횟수", "money_request_count": "금전요구_횟수",
            "resistance_handling_count": "의심저항대응_횟수", "urgency_time_pressure_count": "긴급시간압박_횟수",
            "strategy_diversity": "심리전략종류수", "has_multiple_impersonations": "다중사칭여부",
            "has_bank_impersonation": "은행사칭여부", "has_capital_company_impersonation": "캐피탈사칭여부",
            "has_card_company_impersonation": "카드회사칭여부", "has_financial_association_impersonation": "금융협회사칭여부",
            "has_fss_impersonation": "금융감독원사칭여부", "has_kidnapping_claim_impersonation": "납치주장사칭여부",
            "has_personal_contact_impersonation": "지인사칭여부", "has_police_impersonation": "경찰사칭여부",
            "has_post_office_impersonation": "우체국사칭여부", "has_prosecution_impersonation": "검찰사칭여부",
            "has_avoid_outside_contact": "외부연락차단요구여부", "has_deliver_cash": "현금전달요구여부",
            "has_disclose_account": "계좌정보요구여부", "has_disclose_id_number": "신원정보요구여부",
            "has_rent_account": "계좌대여요구여부", "has_repay_loan": "대출상환요구여부",
            "has_transfer_money": "송금요구여부", "has_visit_atm": "ATM방문요구여부",
            "has_visit_bank": "은행방문요구여부", "has_withdraw_cash": "현금인출요구여부",
        }
        translated = dashboard_df.rename(columns=dashboard_rename)
        extra_columns = ["사건ID"] + [
            column for column in translated.columns
            if column not in korean_result.columns and column != "파일ID"
        ]
        korean_result = korean_result.merge(translated[extra_columns], on="사건ID", how="left")
...
def save_table(
    ...
    existing_paths = [path for path in (csv_path, parquet_path) if path.exists()]
    if skip_existing and existing_paths:
        existing = existing_paths[0]
        print(f"[건너뜀] 기존 파일 유지: {existing.name}")
        return {
            "name": name,
            "rows": int(len(df)),
...
def validate_outputs(tables: dict[str, pd.DataFrame]) -> dict:
    ...
    foreign_key_checks = {
        "vp_cases.file_id": ("vp_cases", "file_id", "vp_files", "file_id"),
        "vp_utterances.case_id": ("vp_utterances", "case_id", "vp_cases", "case_id"),
        "vp_utterances.file_id": ("vp_utterances", "file_id", "vp_files", "file_id"),
        "vp_impersonations.case_id": ("vp_impersonations", "case_id", "vp_cases", "case_id"),
        "vp_requested_actions.case_id": ("vp_requested_actions", "case_id", "vp_cases", "case_id"),
        "vp_strategy_events.case_id": ("vp_strategy_events", "case_id", "vp_cases", "case_id"),
        "vp_amount_events.case_id": ("vp_amount_events", "case_id", "vp_cases", "case_id"),
        "vp_impersonations.evidence_turn_id": ("vp_impersonations", "evidence_turn_id", "vp_utterances", "turn_id"),
        "vp_requested_actions.evidence_turn_id": ("vp_requested_actions", "evidence_turn_id", "vp_utterances", "turn_id"),
        "vp_strategy_events.evidence_turn_id": ("vp_strategy_events", "evidence_turn_id", "vp_utterances", "turn_id"),
        "vp_amount_events.evidence_turn_id": ("vp_amount_events", "evidence_turn_id", "vp_utterances", "turn_id"),
    }
    orphan_report = {}
    for check_name, (child_name, child_column, parent_name, parent_column) in foreign_key_checks.items():
        child_df = tables[child_name]
        parent_df = tables[parent_name]
        if child_df.empty or child_column not in child_df or parent_column not in parent_df:
            orphan_report[check_name] = 0
            continue
        parent_values = set(parent_df[parent_column].dropna())
        orphan_report[check_name] = int((~child_df[child_column].isin(parent_values) & child_df[child_column].notna()).sum())
    report["foreign_key_checks"] = orphan_report
    ...
    if not tables["fraud_detection_ml"].empty:
        ...
        report["fraud_detection_ml"]["original_split"] = tables["fraud_detection_ml"]["original_split"].value_counts(dropna=False).to_dict()
    critical_tables = ["vp_files", "vp_cases", "vp_utterances", "normal_finance_calls", "fraud_detection_ml"]
    duplicate_total = sum(item.get("duplicate_id_count", 0) for item in report.values() if isinstance(item, dict))
    missing_total = sum(item.get("missing_id_count", 0) for item in report.values() if isinstance(item, dict))
    orphan_total = sum(orphan_report.values())
    empty_critical = [name for name in critical_tables if tables[name].empty]
    report["integrity_summary"] = {
        "duplicate_id_total": int(duplicate_total),
        "missing_id_total": int(missing_total),
        "foreign_key_orphan_total": int(orphan_total),
        "empty_critical_tables": empty_critical,
        "integrity_passed": duplicate_total == 0 and missing_total == 0 and orphan_total == 0 and not empty_critical,
    }
    return report
...
def main():
    ...
    parser.add_argument(
        "--skip-existing",
        action="store_true",
        help="CSV 또는 Parquet가 있으면 해당 테이블을 덮어쓰지 않습니다.",
    )
    ...
    print(f"데이터셋 구축 스크립트 v{SCRIPT_VERSION}")
    ...
    korean_cases_df = build_korean_case_summary(
        files_df, cases_df, impersonations_df, actions_df, amounts_df, dashboard_df
    )
    ...
    print("영문 코딩용 표와 전체 한글 대시보드 표를 함께 저장합니다.")
    ...
    for name in ["vp_files", "vp_cases", "vp_utterances", "vp_impersonations", "vp_requested_actions", "vp_strategy_events", "vp_amount_events", "normal_finance_calls"]:
        manifest.append(save_table(tables[name], standard_dir, name, args.skip_existing))
    for name in ["fraud_detection_ml", "fraud_type_ml", "segment_detection_ml", "case_clustering_ml"]:
        manifest.append(save_table(tables[name], ml_dir, name, args.skip_existing))
    manifest.append(save_table(
        tables["dashboard_case_summary"], dashboard_dir, "dashboard_case_summary", args.skip_existing
    ))
    for name in ["보이스피싱_사건요약_한글", "우체국_피해사례_표준화", "사기유형_매핑표", "사기유형_통합비교"]:
        manifest.append(save_table(tables[name], dashboard_dir, name, args.skip_existing))

    validation = validate_outputs(tables)
    case_paths = sorted(args.result_root.rglob("cases.json"))
    normal_zip_paths = sorted(args.normal_root.rglob("*.zip")) if args.normal_root and args.normal_root.exists() else []
    labeled_zip_paths = [path for path in normal_zip_paths if path.name.startswith(("TL_", "VL_"))]
    input_paths = case_paths + labeled_zip_paths + ([args.postal_path] if args.postal_path and args.postal_path.exists() else [])
    manifest_payload = {
        "metadata": {
            ...
            "normal_to_fraud_ratio": NORMAL_TO_FRAUD_RATIO,
            "input_counts": {
                "phishing_cases_json": len(case_paths),
                "normal_all_zip": len(normal_zip_paths),
                "normal_labeled_zip_used": len(labeled_zip_paths),
                "postal_rows": int(len(postal_raw_df)),
            },
            "input_inventory_sha256": inventory_fingerprint(input_paths, args.result_root.parent) if input_paths else None,
        },
        "tables": manifest,
    }
    (report_dir / "dataset_manifest.json").write_text(json.dumps(manifest_payload, ensure_ascii=False, indent=2), encoding="utf-8")
