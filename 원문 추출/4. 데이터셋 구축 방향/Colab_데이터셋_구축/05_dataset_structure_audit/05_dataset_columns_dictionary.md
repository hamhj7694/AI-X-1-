...
- 보이스피싱 자료는 전체 통화가 아니라 핵심 구간 발췌본이다. 따라서 시간·길이·발화순서 컬럼은 원래 통화의 길이, 초반·후반, 최초 등장 시점을 의미하지 않으며 품질·정렬 확인용으로만 사용한다.
- 정상 금융상담과 보이스피싱은 수집·전사 형식과 문체가 다를 수 있다. 두 자료의 높은 분류 성능은 범죄 내용이 아니라 데이터 스타일 차이일 가능성을 먼저 검증해야 한다.
- 분석·모델별 상세 사용 기준은 [`05_column_usage_and_data_limitations.md`](./05_column_usage_and_data_limitations.md)를 따른다.
...
7. 시간·길이·순서 컬럼은 발췌본 내부 기록값이며 전체 보이스피싱 통화의 진행 특성 분석에는 사용하지 않습니다.
8. 정상·사기 비교 전에는 어휘 중첩, 형식 전용 분류, 정규화 전후 성능 비교로 출처·스타일 교란을 확인해야 합니다.
